...
def fetch_crime_window(ti):
    select_old = """
        ...
        """
    select_statement = """select max(to_char(window_end, 'YYYY-MM-DD"T"HH24:MI:ss.ms')) from public.crime_2021_raw;"""
...
def download_latest_crime_data(ti):
    ...
    filter_str = "report_date BETWEEN '{}' and '{}'".format(
...
def load_data():
    ...
    with open(tot_name, 'r') as inputfile:
        ...
        for doc in this_doc:
            ...
            involvement = str(doc['involvement']
                              ) if 'involvement' in doc else ''
            race = str(doc['race']) if 'race' in doc else ''
            sex = str(doc['sex']) if 'sex' in doc else ''
            age = int(doc['age']) if 'age' in doc else ''
            firearmused_flag = str(
                doc['firearmused_flag']) if 'firearmused_flag' in doc else ''
            address_coord = str(
                doc['location']['coordinates']) if 'location' in doc else ''

            # slow by slow, row by row insert
            # TODO convert to bulk inset
            row = (report_no, report_date, report_time, from_date, from_time, to_time, offense, ibrs, description, beat,
                   address, city, zip_code, rep_dist, area, dvflag, involvement, race, sex, age, firearmused_flag, address_coord)
